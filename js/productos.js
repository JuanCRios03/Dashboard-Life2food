...
function displayProducts(products) {
    ...
    tbody.innerHTML = products.map(product => `
        <tr>
            ...
            <td>$${product.price || product.precio || 0}</td>
            <td>${product.category || product.categoria || 'Sin categoría'}</td>
            <td>${product.stock || 0}</td>
            <td>
                <span class="badge ${(product.stock || 0) > 10 ? 'badge-success' : 'badge-warning'}">
                    ${(product.stock || 0) > 10 ? 'Disponible' : 'Bajo stock'}
                </span>
            </td>
            <td>
