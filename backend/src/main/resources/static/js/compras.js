...
function displayPurchases(page) {
    ...
    if (allPurchases.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" style="text-align: center; padding: 40px;">No hay compras registradas</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = purchasesToShow.map((purchase, index) => {
        ...
        const statusKey = getOrderStatusKey(item);
        const statusLabel = getOrderStatusLabel(statusKey);
        const statusClass = getOrderStatusClass(statusKey);
...
function getOrderStatusKey(item) {
    const raw = (item.status || item.state || item.orderStatus || '').toString().toLowerCase();
    if (raw.includes('complete') || raw.includes('completed') || raw.includes('complet')) {
        return 'completed';
    }
...
function getOrderStatusLabel(statusKey) {
    switch (statusKey) {
        case 'completed':
            return 'Completada';
        case 'cancelled':
...
function getOrderStatusClass(statusKey) {
    switch (statusKey) {
        case 'completed':
